...
class SimpleConsumer(Consumer):
    ...
    def __init__(
        ...
        if max_buffer_size is not None and buffer_size > max_buffer_size:
            raise ValueError(
                "buffer_size (%d) is greater than "
                "max_buffer_size (%d)" % (buffer_size, max_buffer_size)
            )
    ...
    def reset_partition_offset(self, partition):
        ...
        # send_offset_request
        log.info(
            "Resetting topic-partition offset to %s for %s:%d",
        ...
        try:
            ...
            log.error(
                "%s sending offset request for %s:%d",
    ...
    def seek(self, offset, whence=None, partition=None):
        ...
        if whence is None:  # set an absolute offset
            ...
            if partition is None:
                ...
                for tmp_partition, r in izip_longest(
                    self.offsets.keys(), repeat(1, rem), fillvalue=0
                ):
                ...
                for tmp_partition in self.offsets.keys():
                    if whence == 0:
                        reqs.append(
                            OffsetRequestPayload(self.topic, tmp_partition, -2, 1)
                        )
                    elif whence == 2:
                        reqs.append(
                            OffsetRequestPayload(self.topic, tmp_partition, -1, 1)
                        )
                    else:
                        pass
            else:
    ...
    def _get_message(
        self, block=True, timeout=0.1, get_partition_info=None, update_offset=True
    ):
        """
        ...
        while partitions:
            ...
            for resp in responses:
                ...
                try:
                    ...
                    log.error(
                        "UnknownTopicOrPartitionError for %s:%d",
                        resp.topic,
                        resp.partition,
                    )
                ...
                if resp.messages and isinstance(
                    resp.messages[-1].message, PartialMessage
                ):
                    ...
                    log.warning(
                        "Fetch size too small, increase to %d (2x) " "and retry",
                        buffer_size,
                    )
