...
class KafkaProtocol(object):
    ...
    @classmethod
    def _encode_message_header(cls, client_id, correlation_id, request_key, version=0):
        """
    ...
    def encode_produce_request(cls, payloads=(), acks=1, timeout=1000):
        ...
        return kafka.protocol.produce.ProduceRequest[0](
            required_acks=acks, timeout=timeout, topics=topics
        )
    ...
    def decode_fetch_response(cls, response):
        ...
        return [
            kafka.structs.FetchResponsePayload(
                ...
                error,
                highwater_offset,
                [offset_and_msg for offset_and_msg in cls.decode_message_set(messages)],
            )
            for topic, partitions in response.topics
    ...
    def decode_message_set(cls, messages):
        for offset, _, message in messages:
            if (
                isinstance(message, kafka.protocol.message.Message)
    ...
    def decode_list_offset_response(cls, response):
        ...
        """
        return [
            kafka.structs.ListOffsetResponsePayload(
    ...
    def encode_consumer_metadata_request(cls, client_id, correlation_id, payloads):
        ...
        message = []
        message.append(
            cls._encode_message_header(
                client_id, correlation_id, KafkaProtocol.CONSUMER_METADATA_KEY
            )
        )
    ...
    def encode_offset_fetch_request(cls, group, payloads, from_kafka=False):
        ...
        return kafka.protocol.commit.OffsetFetchRequest[version](
            consumer_group=group,
            topics=[
                (topic, list(topic_payloads.keys()))
                for topic, topic_payloads in six.iteritems(
                    group_by_topic_and_partition(payloads)
                )
            ],
