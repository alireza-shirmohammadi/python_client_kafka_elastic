...
class SimpleClient(object):
    ...
    def _send_broker_unaware_request(self, payloads, encoder_fn, decoder_fn):
        ...
        for (host, port, afi) in hosts:
            try:
                ...
                log.warning(
                    "Skipping unconnected connection: %s:%s (AFI %s)", host, port, afi
                )
                continue
    ...
    def _send_consumer_aware_request(self, group, payloads, encoder_fn, decoder_fn):
        ...
        request = encoder_fn(
            client_id=self.client_id, correlation_id=request_id, payloads=payloads
        )
        ...
        try:
            ...
            log.warning(
                "ConnectionError attempting to send request %s " "to server %s: %s",
                request_id,
                broker,
                e,
            )
        ...
        # No exception, try to get response
            ...
            if decoder_fn is None:
                log.debug(
                    "Request %s does not expect a response " "(skipping conn.recv)",
                    request_id,
                )
                for payload in payloads:
                    topic_partition = (payload.topic, payload.partition)
        ...
        for error, topic, partitions in resp.topics:
            ...
            if error:
                ...
                if error_type in (
                    ...
                    log.error(
                        "Error loading topic metadata for %s: %s (%s)",
                        topic,
                        error_type,
                        error,
                    )
                    if topic not in topics:
                        ...
                        error_type is LeaderNotAvailableError
            ...
            for error, partition, leader, _, _ in partitions:
                ...
                if error:
                    ...
                    if error_type is LeaderNotAvailableError:
                        log.error(
                            "No leader for topic %s partition %d", topic, partition
                        )
    ...
    def send_metadata_request(self, payloads=(), fail_on_error=True, callback=None):
    ...
    def send_consumer_metadata_request(
        self, payloads=(), fail_on_error=True, callback=None
    ):
    ...
    def send_produce_request(
        self, payloads=(), acks=1, timeout=1000, fail_on_error=True, callback=None
    ):
        """
        ...
        encoder = functools.partial(
            KafkaProtocol.encode_produce_request, acks=acks, timeout=timeout
        )
        ...
        return [
            resp if not callback else callback(resp)
            for resp in resps
            if resp is not None
            and (not fail_on_error or not self._raise_on_response_error(resp))
        ]
    ...
    def send_fetch_request(
        ...
        resps = self._send_broker_aware_request(
            payloads, encoder, KafkaProtocol.decode_fetch_response
        )
    ...
    def send_list_offset_request(self, payloads=(), fail_on_error=True, callback=None):
        resps = self._send_broker_aware_request(
            payloads,
    ...
    def send_offset_commit_request(
        self, group, payloads=(), fail_on_error=True, callback=None
    ):
        encoder = functools.partial(
            KafkaProtocol.encode_offset_commit_request, group=group
        )
    ...
    def send_offset_fetch_request(
        ...
        encoder = functools.partial(
            KafkaProtocol.encode_offset_fetch_request, group=group
        )
    ...
    def send_offset_fetch_request_kafka(
        ...
        encoder = functools.partial(
            KafkaProtocol.encode_offset_fetch_request, group=group, from_kafka=True
        )
        decoder = KafkaProtocol.decode_offset_fetch_response
