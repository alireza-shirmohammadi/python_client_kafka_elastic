...
def _send_upstream(
    ...
    while not (stop_event.is_set() and queue.empty() and not request_tries):
        ...
        for topic_partition, msg in msgset.items():
            ...
            req = ProduceRequestPayload(
                topic_partition.topic, topic_partition.partition, tuple(messages)
            )
            request_tries[req] = 0
        ...
        responses = client.send_produce_request(
            requests, acks=req_acks, timeout=ack_timeout, fail_on_error=False
        )
        ...
        if retry_state["do_backoff"] and retry_options.backoff_ms:
            log.warn(
                "Async producer backoff for %s(ms) before retrying",
                retry_options.backoff_ms,
            )
            time.sleep(float(retry_options.backoff_ms) / 1000)
    ...
    if request_tries or not queue.empty():
        log.error(
            "Stopped producer with %d unsent messages",
...
class Producer(object):
    ...
    def stop(self, timeout=None):
        ...
        if timeout is not None:
            log.warning(
                "timeout argument to stop() is deprecated - "
