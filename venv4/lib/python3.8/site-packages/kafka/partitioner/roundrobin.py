...
class RoundRobinPartitioner(Partitioner):
    ...
    def __call__(self, key, all_partitions=None, available_partitions=None):
        ...
        if not self.partitions:
            ...
            cur_partitions != self.partitions_iterable.partitions
            and cur_partitions is not None
        ):
            self._set_partitions(cur_partitions)
...
class CachedPartitionCycler(object):
    ...
    def next(self):
        ...
        if self.cur_pos is None or not self._index_available(
            self.cur_pos, self.partitions
        ):
            self.cur_pos = 1
