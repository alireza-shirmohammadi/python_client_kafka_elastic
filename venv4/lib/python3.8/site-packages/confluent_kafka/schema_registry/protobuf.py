...
from .schema_registry_client import Schema, SchemaReference
        ...
        if not isinstance(message_type, self._msg_class):
            raise ValueError(
                "message must be of type {} not {}".format(
                    self._msg_class, type(message_type)
                )
            )

        subject = self._subject_name_func(ctx, message_type.DESCRIPTOR.full_name)
